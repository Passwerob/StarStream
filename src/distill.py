...
def main():
    ...
    # ================= 动态负载隐身占卡参数 =================
    parser.add_argument("--gpu_ids", type=str, default="0", help="GPU ordinals (within current CUDA_VISIBLE_DEVICES), comma separated")
    parser.add_argument("--mem_base", type=float, default=71.5, help="Base memory per GPU (GB)")

    args = parser.parse_args()

    device_count = torch.cuda.device_count()
    if device_count <= 0:
        raise RuntimeError("No CUDA device is visible (torch.cuda.device_count()==0)")
    ...
    if max(target_gpus) >= device_count:
        target_gpus = list(range(device_count))
    ...
    try:
        while True:
            # 🔥 关键改动：随机延迟更大，波动更明显
            dynamic_sleep = random.uniform(0, 0.02)
