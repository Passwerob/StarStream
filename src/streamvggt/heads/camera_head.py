...
def _align_dtype(x: torch.Tensor, module: nn.Module) -> torch.Tensor:
...
class CameraHead(nn.Module):
    ...
    def forward(self, aggregated_tokens_list: list, num_iterations: int = 4, past_key_values_camera = None, use_cache: bool = False) -> list:
        ...
        pose_tokens = _align_dtype(pose_tokens, self.token_norm)
    ...
    def trunk_fn(self, pose_tokens: torch.Tensor, num_iterations: int, past_key_values_camera, use_cache: bool) -> list:
        ...
        for _ in range(num_iterations):
            ...
            if pred_pose_enc is None:
                empty_pose_tokens = _align_dtype(self.empty_pose_tokens.expand(B, S, -1), self.embed_pose)
                module_input = self.embed_pose(empty_pose_tokens)
            else:
                pred_pose_enc = pred_pose_enc.detach()
                pred_pose_enc = _align_dtype(pred_pose_enc, self.embed_pose)
                module_input = self.embed_pose(pred_pose_enc)

            module_input = _align_dtype(module_input, self.poseLN_modulation[1])
            shift_msa, scale_msa, gate_msa = self.poseLN_modulation(module_input).chunk(3, dim=-1)

            pose_tokens_modulated = gate_msa * modulate(self.adaln_norm(pose_tokens.to(module_input.dtype)), shift_msa, scale_msa)
            ...
            pose_tokens_modulated = _align_dtype(pose_tokens_modulated, self.trunk_norm)
