...
def render(
    ...
    rendered_rgbs = torch.cat(rendered_rgbs, dim=0)    # (B, H, W, 3)
    accs = torch.cat(accs, dim=0)                      # (B, H, W, 1)
...
def get_render_results(gts, preds, self_view=False):
    ...
    with torch.no_grad():
        ...
        for i, (gt, pred) in enumerate(zip(gts, preds)):
            ...
            pred_rgb, depth, _ = render(intrinsics, pred, gt_img)
            gt_rgb, gt_depth, _ = render(intrinsics, geotrf(camera, gt_pts3d), gt_img)
            depths.append(depth)
            gt_depths.append(gt_depth)
            rendered_imgs.append(pred_rgb)
            gt_rendered_imgs.append(gt_rgb)
    return depths, gt_depths, rendered_imgs, gt_rendered_imgs
