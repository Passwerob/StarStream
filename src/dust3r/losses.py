...
def check_and_fix_inf_nan(x: torch.Tensor, name: str | None = None) -> torch.Tensor:
...
class DepthOrPmapLoss(nn.Module):
    ...
    def forward(self, pred, gt, sigma_p=None, sigma_g=None, valid_mask=None):
        if valid_mask is None:
            valid_mask = torch.ones(pred.shape[:3], dtype=torch.bool, device=pred.device)
        valid_mask = valid_mask.to(dtype=torch.bool, device=pred.device)

        pred = torch.nan_to_num(pred, nan=0.0, posinf=0.0, neginf=0.0)
        gt = torch.nan_to_num(gt, nan=0.0, posinf=0.0, neginf=0.0)
        valid_mask = valid_mask & torch.isfinite(pred).all(dim=-1) & torch.isfinite(gt).all(dim=-1)

        if sigma_p is None:
            sigma_p = torch.ones(pred.shape[:3], dtype=pred.dtype, device=pred.device)
        if sigma_p is not None:
            sigma_p = torch.nan_to_num(sigma_p, nan=1.0, posinf=1.0, neginf=1.0)
            valid_mask = valid_mask & torch.isfinite(sigma_p)
        if sigma_g is not None:
            sigma_g = torch.nan_to_num(sigma_g, nan=1.0, posinf=1.0, neginf=1.0)
            valid_mask = valid_mask & torch.isfinite(sigma_g)
        ...
        # Incorporate teacher confidence: when teacher is less confident,
        # downweight the loss for those pixels (sigma_g is detached since
        # the teacher is frozen and should not receive gradients).
        if sigma_g is not None:
            sigma = sigma_p * sigma_g.detach()
        else:
            sigma = sigma_p
        ...
        weighted = sigma[..., None].expand(-1, -1, -1, C) * diff
        sel = weighted[valid_mask[..., None].expand(-1, -1, -1, C)]
        main_loss = sel.mean() if sel.numel() > 0 else pred.new_zeros(())
        ...
        # Regularize only student confidence (sigma_p), not the combined sigma,
        # so the teacher's confidence does not distort the student's learning signal.
        reg_sel = (-self.alpha * torch.log(sigma_p.clamp(min=1e-6)))[valid_mask]
...
class TrackLoss(nn.Module):
    ...
    def forward(self, y_pr, y_gt, vis_pr, vis_gt, w_p, w_g):
        # Incorporate teacher confidence: downweight tracking loss where
        # the teacher is less confident (w_g detached since teacher is frozen).
        if w_g is not None:
            w = w_p * w_g.detach()
        else:
            w = w_p
        ...
        # Regularize only student confidence (w_p), not the combined weight
        reg = -self.conf_reg_alpha * torch.log(w_p.clamp(min=1e-6))
        reg = reg.mean()

        total = l_pos + l_vis + reg
        return torch.nan_to_num(total, nan=0.0, posinf=0.0, neginf=0.0)

def extri_intri_to_pose_encoding(extrinsics_w2c, intrinsics, images_hw):
    """Convert GT extrinsics (world-to-camera) and intrinsics to 9D pose encoding.

    Delegates to ``streamvggt.utils.pose_enc.extri_intri_to_pose_encoding``
    which produces the exact encoding the model's camera_head outputs:
        [w2c_translation(3), XYZW_quaternion(4), FoV(2)]

    Args:
        extrinsics_w2c: [B, V, 4, 4] or [B, V, 3, 4] world-to-camera matrices
        intrinsics: [B, V, 3, 3] camera intrinsic matrices
        images_hw: tuple (H, W) image spatial dimensions

    Returns:
        pose_encoding: [B, V, 9]
    """
    from streamvggt.utils.pose_enc import (
        extri_intri_to_pose_encoding as _official_enc,
    )
    # The official function expects [B, V, 3, 4]; slice off the bottom row
    # if we receive full 4x4 matrices.
    ext = extrinsics_w2c[..., :3, :]  # [B, V, 3, 4]
    return _official_enc(ext, intrinsics, image_size_hw=images_hw)
...
class FinetuneLoss(MultiLoss):
    ...
    def compute_loss(self, gts, preds,
                     track_queries=None, track_preds=None, **kwargs):
        # ---------- Lcamera ----------
        T = []
        for g in gts:
            T_c2w = g['camera_pose'] 
...
class DistillLoss(MultiLoss):
    def __init__(self, lambda_camera=20.0, lambda_depth=20.0, lambda_pmap=10.0, lambda_track=0.5):
        super().__init__()
        self.cam_loss = CameraLoss(
            delta=0.1,
            weights=(1.0, 1.0, 0.5)
    ...
    def compute_loss(self, gts, preds,
        ...
        for g,p in zip(gts, preds):
            if ('depth' in g) and ('depth' in p):
                ...
                if valid_mask is None:
                    valid_mask = torch.ones_like(sigma_p, dtype=torch.bool)
        ...
        total = self.lambda_camera * Lcamera + self.lambda_depth * Ldepth + self.lambda_pmap * Lpmap + self.lambda_track * Ltrack
        details = {}

        details['Lcamera'] = float(Lcamera) * self.lambda_camera
        details['Ldepth'] = float(Ldepth) * self.lambda_depth
        details['Lpmap'] = float(Lpmap) * self.lambda_pmap
        details['Ltrack'] = float(Ltrack) * self.lambda_track
        details['total'] = float(total)

        return total, details


class GtPoseDistillLoss(MultiLoss):
    """Hybrid supervision:

    - Camera:  GT c2w from dataset (``camera_pose_gt`` + ``camera_intrinsics_gt``).
               Falls back to teacher pseudo-label when GT is unavailable.
    - Depth:   DA3 pseudo-GT (``da3_disparity``) via a scale/shift-invariant
               loss in disparity space. Falls back to teacher pseudo-label.
    - Pmap:    Always teacher distillation (no GT currently available; a
               synthesised GT from pose+depth is a future extension).
    - Track:   Always teacher distillation.
    - PoseSC:  Self-supervised pose chain-consistency (triplet SE3).
    - Evt:     Event brightness-constancy loss (physical model).
    """

    def __init__(self, lambda_camera=20.0, lambda_depth=20.0,
                 lambda_pmap=10.0, lambda_track=0.5,
                 lambda_pose_sc=0.3, lambda_evt=0.15,
                 warmup_pose_sc=5, warmup_evt=5):
        super().__init__()
        ...
        self.lambda_pmap = lambda_pmap
        self.lambda_track = lambda_track
        self.lambda_pose_sc = lambda_pose_sc
        self.lambda_evt = lambda_evt
        self.warmup_pose_sc = warmup_pose_sc
        self.warmup_evt = warmup_evt
        self._current_epoch = 0
        self._depth_eps = 1e-6

    def get_name(self):
        return "GtPoseDistillLoss"
    ...
    @staticmethod
    def _view_has_gt_pose(v) -> bool:
        return (isinstance(v, dict)
                and torch.is_tensor(v.get('camera_pose_gt'))
                and torch.is_tensor(v.get('camera_intrinsics_gt')))

    @staticmethod
    def _view_has_da3(v) -> bool:
        return isinstance(v, dict) and torch.is_tensor(v.get('da3_disparity'))

    @staticmethod
    def _any_gt_pose(data_views) -> bool:
        return bool(data_views) and any(
            GtPoseDistillLoss._view_has_gt_pose(v) for v in data_views
        )

    @staticmethod
    def _any_da3(data_views) -> bool:
        return bool(data_views) and any(
            GtPoseDistillLoss._view_has_da3(v) for v in data_views
        )

    def _camera_loss_gt(self, data_views, preds):
        """Per-view GT camera loss — first-frame-relative w2c, then 9D encoding.

        Views missing ``camera_pose_gt`` / ``camera_intrinsics_gt`` are skipped.
        If the first frame (view 0) has no GT, returns ``None`` (caller should
        fall back to teacher for camera loss entirely).
        """
        if not self._view_has_gt_pose(data_views[0]):
            return None

        has_gt = [self._view_has_gt_pose(v) for v in data_views]

        gt_views = [v for v, ok in zip(data_views, has_gt) if ok]
        gt_preds = [p for p, ok in zip(preds, has_gt) if ok]
        if not gt_views:
            return None

        T_c2w = torch.stack(
            [v['camera_pose_gt'].float() for v in gt_views], dim=1
        )  # [B, V_gt, 4, 4]
        dtype = T_c2w.dtype
        device = T_c2w.device
        ...
        t_w2c = -(R_w2c @ t)
        eye = torch.eye(4, dtype=dtype, device=device)
        T_w2c = eye.expand(*T_c2w.shape[:-2], 4, 4).clone()
        T_w2c[..., :3, :3] = R_w2c
        T_w2c[..., :3, 3:4] = t_w2c

        T_c2w_first = T_c2w[:, 0]  # [B, 4, 4]
        T_wprime2c = T_w2c @ T_c2w_first.unsqueeze(1)

        intrinsics = torch.stack(
            [v['camera_intrinsics_gt'].float() for v in gt_views], dim=1
        )
        images_hw = data_views[0]['img'].shape[-2:]
        cam_gt = extri_intri_to_pose_encoding(T_wprime2c, intrinsics, images_hw)
        cam_pr = torch.stack([p['camera_pose'] for p in gt_preds], dim=1)
        return self.cam_loss(cam_pr, cam_gt)

    def _camera_loss_teacher(self, gts, preds):
        cam_gt = torch.stack([g['camera_pose'] for g in gts], dim=1)
        cam_pr = torch.stack([p['camera_pose'] for p in preds], dim=1)
        return self.cam_loss(cam_pr, cam_gt)

    def _depth_loss_da3(self, data_views, preds):
        """DA3 disparity pseudo-GT supervision.

        Student head outputs depth (``p['depth']``, positive, up-to-scale).
        DA3 gives relative disparity (``v['da3_disparity']``, up-to-scale-and-shift).
        We map student depth to disparity via ``1/(d+eps)`` so both tensors
        live in the same (disparity) space, then rely on the existing
        closed-form scale/shift alignment inside ``DepthOrPmapLoss``.
        """
        terms = []
        for v, p in zip(data_views, preds):
            if not torch.is_tensor(v.get('da3_disparity')):
                continue
            if 'depth' not in p:
                continue
            pred_depth = p['depth']  # [B, H, W, 1]
            sigma_p = p['depth_conf']  # [B, H, W]

            pred_disp = 1.0 / pred_depth.clamp(min=self._depth_eps)
            gt_disp = v['da3_disparity'].to(dtype=pred_disp.dtype,
                                            device=pred_disp.device)
            if gt_disp.dim() == 3:  # [B, H, W] -> [B, H, W, 1]
                gt_disp = gt_disp.unsqueeze(-1)

            da3_mask = v.get('da3_valid_mask', None)
            if da3_mask is not None:
                da3_mask = da3_mask.to(device=pred_disp.device)
            valid_mask = v.get('valid_mask', None)
            if valid_mask is None:
                valid_mask = torch.ones_like(sigma_p, dtype=torch.bool)
            elif not valid_mask.any():
                valid_mask = torch.ones_like(valid_mask)
            if da3_mask is not None:
                valid_mask = valid_mask & da3_mask
            if not valid_mask.any():
                continue

            terms.append(self.depth_loss(
                pred_disp, gt_disp,
                sigma_p=sigma_p, sigma_g=None,
    ...
    def _depth_loss_teacher(self, gts, preds):
        terms = []
        for g, p in zip(gts, preds):
            if ('depth' not in g) or ('depth' not in p):
                continue
            sigma_p = p['depth_conf']
            sigma_g = g['depth_conf']
            valid_mask = g.get('valid_mask', None)
            if valid_mask is None:
                ...
                valid_mask = torch.ones_like(valid_mask)
            terms.append(self.depth_loss(p['depth'], g['depth'],
                                         sigma_p, sigma_g, valid_mask))
        if not terms:
            return None
        return torch.stack(terms).mean()

    def _pmap_loss_teacher(self, gts, preds):
        terms = []
        for g, p in zip(gts, preds):
            if ('pts3d_in_other_view' not in g) or ('pts3d_in_other_view' not in p):
                continue
            sigma_p = p['conf']
            sigma_g = g['conf']
            valid_mask = g.get('valid_mask', None)
            if valid_mask is None:
                ...
                valid_mask = torch.ones_like(valid_mask)
            terms.append(self.pmap_loss(
                p['pts3d_in_other_view'], g['pts3d_in_other_view'],
                sigma_p, sigma_g, valid_mask,
            ))
        if not terms:
            return None
    ...
    def _track_loss_teacher(self, gts, preds):
        if not (('track' in gts[0]) and ('track' in preds[0])):
            return None
        y_gt = torch.stack([g['track'] for g in gts], dim=1)
        vis_gt = torch.stack([g['vis'] for g in gts], dim=1)
        y_pr = torch.stack([p['track'] for p in preds], dim=1)
    ...
    # ------------------------------------------------------------------
    #  L_pose_sc — Pose Self-Consistency (triplet chain on SE3)
    # ------------------------------------------------------------------
    @staticmethod
    def _se3_log_distance(T1, T2):
        """SE3 distance = ‖Δt‖₂ + ‖log(ΔR)‖₂ between [*, 3, 4] transforms."""
        R1, t1 = T1[..., :3, :3], T1[..., :3, 3]
        R2, t2 = T2[..., :3, :3], T2[..., :3, 3]
        dt = (t1 - t2).norm(dim=-1)
    ...
    def _pose_sc_loss(self, preds, data_views):
        """Temporal acceleration regularizer on student's predicted poses.

        Only activates on sequential (video) clips — the constant-velocity
        assumption is meaningless for randomly sampled frames.
        """
        from streamvggt.utils.pose_enc import pose_encoding_to_extri_intri

        if not all(v.get('is_video', False) for v in data_views):
            return None
        ...
        cam_9d = torch.stack([p['camera_pose'] for p in preds], dim=1)
        images_hw = data_views[0]['img'].shape[-2:]
        ...
        for i in range(V - 2):
            j, k = i + 1, i + 2
            Ri, ti = ext[:, i, :3, :3], ext[:, i, :3, 3:]  # [B,3,3], [B,3,1]
            Rj, tj = ext[:, j, :3, :3], ext[:, j, :3, 3:]
            Rk, tk = ext[:, k, :3, :3], ext[:, k, :3, 3:]

            # V_{ij} = E_j @ E_i^{-1}
            Ri_inv = Ri.transpose(-1, -2)
            R_ij = Rj @ Ri_inv
            t_ij = tj - R_ij @ ti

            # V_{jk} = E_k @ E_j^{-1}
            Rj_inv = Rj.transpose(-1, -2)
            R_jk = Rk @ Rj_inv
            t_jk = tk - R_jk @ tj

            T_ij = torch.cat([R_ij, t_ij], dim=-1)  # [B, 3, 4]
            T_jk = torch.cat([R_jk, t_jk], dim=-1)

            terms.append(self._se3_log_distance(T_ij, T_jk))
    ...
    # ------------------------------------------------------------------
    #  L_evt — Event Brightness-Constancy
    # ------------------------------------------------------------------
    @staticmethod
    def _linearize_srgb(img):
        """sRGB [0,1] → linear [0,1] via inverse gamma."""
        low = img / 12.92
        high = ((img + 0.055) / 1.055).clamp(min=0.0) ** 2.4
        return torch.where(img <= 0.04045, low, high)

    @staticmethod
    def _local_znorm(x, kernel_size=32):
        """Local z-normalization using a sliding window mean/std.

        Uses an asymmetric pad so that the output spatial size exactly
        matches the input for both even and odd kernels. For a kernel of
        size ``k`` and stride 1, the required padding totals ``k - 1``;
        we split it as ``(k-1)//2`` on the left/top and ``k//2`` on the
        right/bottom.
        """
        x4 = x.unsqueeze(1) if x.dim() == 3 else x  # [B, 1, H, W]
        pad_a = (kernel_size - 1) // 2   # left / top
        pad_b = kernel_size // 2         # right / bottom
        pad_spec = [pad_a, pad_b, pad_a, pad_b]  # F.pad order: (L, R, T, B)
        xp = F.pad(x4, pad_spec, mode='reflect')
        local_mean = F.avg_pool2d(xp, kernel_size, stride=1)
        sq_p = F.pad(x4 ** 2, pad_spec, mode='reflect')
        local_sq_mean = F.avg_pool2d(sq_p, kernel_size, stride=1)
        local_var = (local_sq_mean - local_mean ** 2).clamp(min=1e-8)
        out = (x4 - local_mean) / local_var.sqrt()
        return out.squeeze(1) if x.dim() == 3 else out

    def _evt_loss(self, data_views, preds):
        """Event brightness-constancy loss.

        Only activates on sequential (video) clips — the physical model
        requires temporally ordered frames so the event voxel between
        frame s and s+1 matches the predicted depth/pose relationship.
        """
        from streamvggt.utils.pose_enc import pose_encoding_to_extri_intri
        ...
        cam_9d = torch.stack([p['camera_pose'] for p in preds], dim=1)
        images_hw = data_views[0]['img'].shape[-2:]
        ext, intri = pose_encoding_to_extri_intri(
            cam_9d, image_size_hw=images_hw, build_intrinsics=True,
        )
        ...
        for s in range(V - 1):
            ...
            evt_raw = v_t.get('event_voxel_raw')
            if not torch.is_tensor(evt_raw):
                if not torch.is_tensor(v_t.get('event_voxel')):
                    continue
                evt_raw = v_t['event_voxel']
            if 'depth' not in p_s:
                continue

            img_s = v_s['img']  # [B, 3, H, W] in [0, 1]
            evt = evt_raw  # [B, C_evt, H, W] — UN-normalized, physical polarity preserved
            depth_s = p_s['depth']  # [B, H, W, 1]
            device = depth_s.device

            img_s = img_s.to(device)
            evt = evt.to(device).float()
            depth_hw = depth_s.squeeze(-1)  # [B, H, W]

            Es = ext[:, s]      # [B, 3, 4]
            Et = ext[:, s + 1]
            K = intri[:, s]     # [B, 3, 3]

            K_diag_min = K.diagonal(dim1=-2, dim2=-1).abs().min()
            if K_diag_min < 1e-4:
                continue

            Rs, ts = Es[..., :3, :3], Es[..., :3, 3]   # Rs: [B,3,3], ts: [B,3]
            Rt, tt = Et[..., :3, :3], Et[..., :3, 3]   # Rt: [B,3,3], tt: [B,3]
            Rs_inv = Rs.transpose(-1, -2)
            R_s2t = Rt @ Rs_inv                         # [B, 3, 3]
            # ts is [B, 3]; promote to [B, 3, 1] for matmul, then squeeze back
            t_s2t = tt - (R_s2t @ ts.unsqueeze(-1)).squeeze(-1)   # [B, 3]

            B, H, W = depth_hw.shape
            ...
            ones = torch.ones_like(u_coord)
            uv1 = torch.stack([u_coord, v_coord, ones], dim=-1)

            K_inv = torch.linalg.inv(K)
            rays = torch.einsum('bij,hwj->bhwi', K_inv, uv1)
            pts = depth_hw.unsqueeze(-1) * rays
            pts_t = torch.einsum('bij,bhwj->bhwi', R_s2t, pts) + t_s2t[:, None, None, :]
            z_t = pts_t[..., 2:3].clamp(min=1e-6)
            pts_2d = torch.einsum('bij,bhwj->bhwi', K, pts_t)
            pts_2d = pts_2d[..., :2] / z_t
            uv = torch.stack([u_coord, v_coord], dim=-1).unsqueeze(0).expand(B, -1, -1, -1)
            flow = pts_2d - uv  # [B, H, W, 2]

            inbound = (
                (pts_2d[..., 0] >= 0) & (pts_2d[..., 0] < W)
                & (pts_2d[..., 1] >= 0) & (pts_2d[..., 1] < H)
                & (pts_t[..., 2] > 0) & (depth_hw > self._depth_eps)
            )

            linear_rgb = self._linearize_srgb(img_s.clamp(min=1e-6))
            lum = 0.2126 * linear_rgb[:, 0] + 0.7152 * linear_rgb[:, 1] + 0.0722 * linear_rgb[:, 2]
            log_lum = torch.log(lum.clamp(min=1e-6))  # [B, H, W]

            grad_x = F.pad(log_lum[:, :, 1:] - log_lum[:, :, :-1], (0, 1), mode='replicate')
            grad_y = F.pad(log_lum[:, 1:, :] - log_lum[:, :-1, :], (0, 0, 0, 1), mode='replicate')

            E_pred = -(grad_x * flow[..., 0] + grad_y * flow[..., 1])  # [B, H, W]

            grad_mag = (grad_x ** 2 + grad_y ** 2).sqrt()
            grad_valid = grad_mag > 1e-3

            C_evt = evt.shape[1]
            evt_pos = evt.clamp(min=0).sum(dim=1)   # [B, H, W]
            evt_neg = (-evt).clamp(min=0).sum(dim=1) # [B, H, W]
            evt_density = (evt_pos + evt_neg) > (0.1 * C_evt)

            mask = inbound & grad_valid & evt_density  # [B, H, W]
            if not mask.any():
                continue

            E_pred_pos = E_pred.clamp(min=0)
            E_pred_neg = (-E_pred).clamp(min=0)

            E_pred_pos_z = self._local_znorm(E_pred_pos, kernel_size=32)
            E_pred_neg_z = self._local_znorm(E_pred_neg, kernel_size=32)
            E_obs_pos_z = self._local_znorm(evt_pos, kernel_size=32)
            E_obs_neg_z = self._local_znorm(evt_neg, kernel_size=32)

            loss_pos = (E_pred_pos_z - E_obs_pos_z).abs()[mask].mean()
            loss_neg = (E_pred_neg_z - E_obs_neg_z).abs()[mask].mean()
            terms.append(loss_pos + loss_neg)
    ...
    def compute_loss(self, gts, preds,
                     ...
                     data_views=None, **kwargs):
        gts = [self._sanitize_teacher(g) for g in gts]

        # Camera: try GT (per-view); fall back to teacher if GT unavailable
        Lcamera = None
        if self._any_gt_pose(data_views):
            Lcamera = self._camera_loss_gt(data_views, preds)
        if Lcamera is None:
            Lcamera = self._camera_loss_teacher(gts, preds)

        # Depth: try DA3 (per-view); fall back to teacher if no valid DA3 term
        Ldepth = None
        if self._any_da3(data_views):
            Ldepth = self._depth_loss_da3(data_views, preds)
        if Ldepth is None:
            Ldepth = self._depth_loss_teacher(gts, preds)
        if Ldepth is None:
            Ldepth = torch.zeros_like(Lcamera)

        # Pmap / track: always teacher
        Lpmap = self._pmap_loss_teacher(gts, preds)
        if Lpmap is None:
            Lpmap = torch.zeros_like(Lcamera)
        Ltrack = self._track_loss_teacher(gts, preds)
        if Ltrack is None:
            Ltrack = torch.zeros_like(Lcamera)

        # Pose self-consistency (warmup gated)
        Lpose_sc = torch.zeros_like(Lcamera)
        if self._current_epoch >= self.warmup_pose_sc and len(preds) >= 3:
            _sc = self._pose_sc_loss(preds, data_views)
            if _sc is not None:
                Lpose_sc = _sc

        # Event brightness-constancy (warmup gated)
        Levt = torch.zeros_like(Lcamera)
        if self._current_epoch >= self.warmup_evt and data_views:
            _ev = self._evt_loss(data_views, preds)
            if _ev is not None:
                Levt = _ev
        ...
        Lpmap = torch.nan_to_num(Lpmap, nan=0.0, posinf=0.0, neginf=0.0)
        Ltrack = torch.nan_to_num(Ltrack, nan=0.0, posinf=0.0, neginf=0.0)
        Lpose_sc = torch.nan_to_num(Lpose_sc, nan=0.0, posinf=0.0, neginf=0.0)
        Levt = torch.nan_to_num(Levt, nan=0.0, posinf=0.0, neginf=0.0)

        total = (self.lambda_camera * Lcamera
                 + self.lambda_depth * Ldepth
                 + self.lambda_pmap * Lpmap
                 + self.lambda_track * Ltrack
                 + self.lambda_pose_sc * Lpose_sc
                 + self.lambda_evt * Levt)

        _n_gt_pose = sum(self._view_has_gt_pose(v) for v in data_views) if data_views else 0
        _n_da3 = sum(self._view_has_da3(v) for v in data_views) if data_views else 0
        details = {
            'Lcamera':   float(Lcamera) * self.lambda_camera,
            'Ldepth':    float(Ldepth) * self.lambda_depth,
            'Lpmap':     float(Lpmap) * self.lambda_pmap,
            'Ltrack':    float(Ltrack) * self.lambda_track,
            'Lpose_sc':  float(Lpose_sc) * self.lambda_pose_sc,
            'Levt':      float(Levt) * self.lambda_evt,
            'n_gt_pose_views': _n_gt_pose,
            'n_da3_views': _n_da3,
            'total':     float(total),
        }
        return total, details
