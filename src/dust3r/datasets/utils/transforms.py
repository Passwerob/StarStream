...
def _adjust_hue_safe(img, hue_factor):
    if hue_factor is None or hue_factor == 0:
        return img
    ...
    delta = int(round(float(hue_factor) * 255.0))
    hsv[..., 0] = ((hsv[..., 0].astype(np.int16) + delta) % 256).astype(np.uint8)
    ...
    def _color_jitter(img):
        for fn_id in fn_idx:
            if fn_id == 0 and brightness_factor is not None:
                ...
                img = _adjust_hue_safe(img, hue_factor)
        return ImgNorm(img)
