...
def loss_of_one_batch(
    ...
    with torch.cuda.amp.autocast(dtype=dtype):
        ...
        # Snapshot the dataset views so that GT-aware losses can still see
        # fields like ``camera_pose_gt`` / ``da3_disparity`` after the teacher
        # swaps ``batch`` for its own ``knowledge.views``. Shallow-copies the
        # list and each dict; inner tensors remain shared (no extra memory).
        data_views = [dict(v) if isinstance(v, dict) else v for v in batch]

        gts = None
        if teacher is not None:
            # Feed the teacher clean images when curriculum low-light is active.
            # ``teacher_img`` is set by the dataset when student sees low-light;
            # if absent (warmup phase or datasets without low-light), this is a
            # no-op and the teacher sees the same ``img`` as the student.
            #
            # ``teacher_img`` comes from the dataset in [-1, 1] (ImgNorm) while
            # ``img`` has already been rescaled to [0, 1] by train.py before
            # entering this function.  We must apply the same transform here.
            _has_teacher_img = (
                isinstance(batch[0], dict) and 'teacher_img' in batch[0]
            )
            if _has_teacher_img:
                for v in batch:
                    v['img'] = (v['teacher_img'] + 1.0) / 2.0
            ...
            with torch.cuda.amp.autocast(enabled=False):
                loss = (
                    criterion(gts, preds, data_views=data_views)
                    if criterion is not None else None
                )
        else:
            with torch.cuda.amp.autocast(enabled=False):
                loss = (
                    criterion(batch, preds, data_views=data_views)
                    if criterion is not None else None
                )

    # ``gts`` exposes teacher-side predictions (9-D pose encoding, depth,
    # ``pts3d_in_other_view``, etc.) so downstream code (e.g. NVS rendering in
    # validate) can reuse them as pseudo-GT without re-running the teacher.
    result = dict(views=batch, pred=preds, loss=loss, gts=gts)
    return result[ret] if ret else result
